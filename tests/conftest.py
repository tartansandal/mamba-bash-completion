...
def bash(request) -> pexpect.spawn:
    ...
    mamba_file = (test_dir / '..' / 'mamba').resolve()
    assert mamba_file.exists()
    ...
    if load_completion_for(bash, cmd):
        ...
        pytest.fail("Could not load bash completion for mamba")

    # Clean up
    assert_bash_exec(
        bash,
        "mamba env remove -y -n 'mamba-bash-comp-testing'",
        want_output=True
    )
    bash.close()
    if logfile:
